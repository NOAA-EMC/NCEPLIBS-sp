...
PROGRAM TEST_SPPAD
  ...
  DO WHICH=1,6
    ...
    IF ((WHICH.EQ.1).AND.(ANY(ABS(Q1-Q2).GT.TINI))) STOP 2
    IF ((WHICH.EQ.2).AND.(ANY(ABS(Q1-Q2).GT.TINI))) STOP 3
    ! Non-pad values (i.e., non-zeros) should be unchanged
    IF (ANY(ABS(Q1-PACK(Q2,Q2>TINI)).GT.TINI)) STOP 4
    IF (WHICH.EQ.4) THEN
      IF (ANY(ABS(Q2-W4REF).GT.TINI)) STOP 5
    ENDIF
    IF (WHICH.EQ.6) THEN
      IF (ANY(ABS(Q2-W6REF).GT.TINI)) STOP 6
    ENDIF
    DEALLOCATE(Q1)
    DEALLOCATE(Q2)
  ENDDO ! WHICH=1,6
