...
C> @param WAVE     - REAL (MX,KMAX) WAVE FIELD IF IDIR>0
C>                WHERE MX=(MAXWV+1)*((IROMB+1)*MAXWV+2)
C> @param GRIDMN   - REAL (KMAX) GLOBAL MEAN IF IDIR<0
C> @param GRIDX    - REAL (IMAX,JMAX,KMAX) GRID X-GRADIENTS (E->W,N->S) IF IDIR<0
C> @param GRIDY    - REAL (IMAX,JMAX,KMAX) GRID Y-GRADIENTS (E->W,N->S) IF IDIR<0
