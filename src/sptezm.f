C> @file
C>
C> Perform simple scalar spherical transforms
...
C> @param[out] WAVE     - REAL (2*MX,KMAX) WAVE FIELD IF IDIR>0
C>                WHERE MX=(MAXWV+1)*((IROMB+1)*MAXWV+2)/2
C> @param[out] GRID     - REAL (IMAX,JMAX,KMAX) GRID FIELD (E->W,N->S) IF IDIR<0
